...
Workspace
└── PannaWorld              ← создаётся сервером во время выполнения
```
...
| `WorldBuilder` | Процедурно создать лобби, арены и служебные Parts | Управлять очередью или счётом |
...
| `ArenaService` | Обнаружить арены, выдавать свободную, освобождать и очищать | Назначать награды |
...
sequenceDiagram
    ...
    C1->>Q: QueueRequest (войти)
    C2->>Q: QueueRequest (войти)
    Q->>A: запрос свободной арены
    A-->>Q: арена или ожидание
    Q->>M: создать матч для пары
...
7. вернуть оставшегося игрока в лобби после предусмотренной паузы.
