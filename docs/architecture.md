...
| `WorldBuilder` | Процедурно создать единственный `BrightDayFootballDistrict`: центральную улицу, дневной Lighting, лобби, шесть тематических комнат с покрытиями/разметкой и служебные Parts | Управлять очередью или счётом |
