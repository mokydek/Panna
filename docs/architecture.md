...
| `BallService` | Kinematic `Anchored`-владение, физические release/action-состояния, server ownership, изоляция арен и sanity-проверка перемещения | Применять произвольную силу или позицию клиента без проверки |
| `BallMath` | Чистые расчёты направления, charge, kinematic target и визуального качения | Читать состояние матча или перемещать Instances |
...
Каждый мяч реплицирует дискретные атрибуты `BallState`, `BallRevision`, `OwnerUserId`, `LastTouchUserId` и `LastAction`. Допустимые состояния: `Free`, `Controlled`, `Contested`, `Shot`, `Flight`, `Bounce`, `Reset`. Отдельный Remote-поток позиций не используется: в `Controlled` сервер обновляет кинематический CFrame закреплённой сферы, а в остальных состояниях Roblox реплицирует server-owned физику.

### Инвариант мяча

`Controlled` — единственное кинематическое состояние. Сервер обнуляет скорости, выключает legacy `PannaDribbleForce`, закрепляет мяч и каждый Heartbeat вычисляет цель перед горизонтальным `HumanoidRootPart.LookVector`. Ground raycast определяет поверхность, экспоненциальное следование сглаживает положение, `Spherecast` ограничивает swept-путь перед препятствиями, а `BallMath.KinematicRollCFrame` обновляет визуальное вращение. Клиент не присылает координаты цели.

Любой выход из `Controlled` проходит общий physical restore: `Anchored = false`, очищаются kinematic position/rotation, network owner остаётся `nil`. Удар/пас/панна/отбор после этого получают серверный импульс; потеря владельца, reset, death/respawn, завершение или detach также не оставляют закреплённый мяч. Переходы между уже физическими `Shot`, `Flight`, `Bounce` и `Free` сохраняют скорость.
