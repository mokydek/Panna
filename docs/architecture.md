...
| `WorldBuilder` | Процедурно создать единственный `NaturalFootballDistrict`: центральную улицу, нейтральный дневной Lighting, лобби, шесть зелёных травяных комнат с разметкой и служебные Parts | Управлять очередью или счётом |
...
| `BallService` | Всегда незакреплённый server-owned мяч, физическое PD-ведение через `VectorForce`/`Torque`, action-импульсы, Magnus-подкрутка, изоляция арен и sanity-проверка перемещения | Применять произвольную силу или позицию клиента без проверки |
| `BallMath` | Чистые расчёты направления, charge, физической цели/PD-ускорения, поворота направления и Magnus-ускорения | Читать состояние матча или перемещать Instances |
...
| клиентский контроллер | Ввод, камера, HUD, modal guide, мобильные кнопки, эффекты и идемпотентный reset transient-состояния/PlayerModule controls | Назначать гол, победу, монеты или рейтинг |
...
Матчевый `Ball` во всех состояниях сохраняет `Anchored = false` и server network ownership (`SetNetworkOwner(nil)`). В `Controlled` сервер каждый Heartbeat строит цель по проверенной и ограниченной горизонтальной `HumanoidRootPart.AssemblyLinearVelocity`, если игрок действительно движется, либо по реплицируемому горизонтальному `HumanoidRootPart.LookVector` при остановке. Активный финт временно применяет собственное проверенное сервером направление варианта относительно facing/принятого ввода. Ground raycast определяет высоту, критически затухающий ограниченный PD-расчёт преобразует ошибку позиции/скорости в `PannaDribbleForce`, а `PannaRollTorque` помогает сфере катиться по направлению движения. Скорость поворота цели, сила коррекции и воздушное влияние ограничены конфигурацией; клиент не присылает координаты цели или силу mover.

`Workspace:Spherecast` используется только как obstacle guard: он сокращает физическую цель перед бортом/стойкой и при длительной блокировке приводит к потере контроля, но никогда не записывает `CFrame` мяча. Первый приём гасит скорость ограниченным импульсом. При ударе, низком пасе, панне или отборе контролирующие `VectorForce`/`Torque` выключаются, а тому же незакреплённому мячу выдаются линейный и при необходимости угловой импульсы. В `Shot`/`Flight`/`Bounce` ограниченное Magnus-ускорение связывает фактические линейную и угловую скорости; переходы сохраняют физический импульс. Reset — отдельная авторитетная операция возврата на `BallSpawn`, а не режим ведения.
...
`Effect` передаёт только презентационные данные: `kind = "Goal" | "Panna" | "Result" | "Message"`, обязательные для матчевого события `matchId`/`arenaId`/`matchRevision`, а также допустимые `title`, `text`, `won`, `canRematch`. Клиент выбирает самый новый контекст по серверной ревизии и не откатывает его при иной очередности репликации атрибутов, `StateUpdate`, `GetSnapshot` и `Effect`; это защищает реванш от запоздалого `Result` и старого snapshot.
...
8. клиентским единым reset отменить charge/Shield/Trap/Dash/pending actions, очистить transient HUD/touch, включить PlayerModule controls и привязать камеру к текущему Humanoid;
9. вернуть оставшегося игрока к `StreetSpawn` комнаты (с запасным возвратом в лобби) после предусмотренной паузы.

Оба reset-пути идемпотентны и вызываются по lifecycle-атрибутам/снимкам, а не по предположению клиента о причине завершения. Их фактическое поведение в двух матчах подряд и при смерти/respawn остаётся обязательной regression-проверкой.
