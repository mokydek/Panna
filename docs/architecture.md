...
| клиентский контроллер | Ввод, камера, HUD, мобильные кнопки, эффекты и идемпотентный reset transient-состояния/PlayerModule controls | Назначать гол, победу, монеты или рейтинг |
...
Матчевый `Ball` во всех состояниях сохраняет `Anchored = false` и server network ownership (`SetNetworkOwner(nil)`). В `Controlled` сервер каждый Heartbeat строит цель перед горизонтальным `HumanoidRootPart.LookVector`: ground raycast определяет высоту, ограниченный PD-расчёт преобразует ошибку позиции/скорости в `PannaDribbleForce`, а `PannaRollTorque` помогает сфере катиться по направлению движения. Скорость поворота цели, сила коррекции и воздушное влияние ограничены конфигурацией; клиент не присылает координаты цели или силу mover.
