...
function FootballVFXController.ApplyEffect(self: FootballVFXController, effect: any): boolean
	...
	if kind == "playeraction" then
		...
		local revisionValue = effect.visualRevision
			or effect.VisualRevision
			or effect.actionRevision
			or effect.ActionRevision
		...
		local isStop = action == "Shield"
			and (mode == "Stop" or mode == "Cancel" or mode == "End" or mode == "Off")
		if
			userId < 1
			or actorValue ~= userId
			or not finiteNumber(revisionValue)
			or revisionValue < 1
			or revisionValue ~= math.floor(revisionValue)
		then
			return true
		end
		...
		local timestampValue = effect.serverTime or effect.ServerTime
		local now = Workspace:GetServerTimeNow()
		local timestampValid = finiteNumber(timestampValue)
			and timestampValue >= now - MAX_EFFECT_AGE
			and timestampValue <= now + MAX_EFFECT_FUTURE
		...
		if action == "Shield" then
			...
			elseif mode == "Stop" or mode == "Cancel" or mode == "End" or mode == "Off" then
				self:_removeShield(userId)
			end
			return true
		...
		local validBall = ball ~= nil and ball.Parent ~= nil
		if not validBall and not root then
			return false
		end
		local origin = if validBall
			then ball.Position
			else (root :: BasePart).Position + Vector3.new(0, 0.8, 0)
	...
	elseif kind == "goal" or kind == "panna" then
		...
		local origin = if validBall
			then ball.Position
			else (root :: BasePart).Position + Vector3.yAxis
